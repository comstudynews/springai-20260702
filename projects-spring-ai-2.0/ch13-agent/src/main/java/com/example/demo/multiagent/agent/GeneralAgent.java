...
public class GeneralAgent {
  ...
  private static final String SYSTEM_PROMPT = """
      ...
      지식 베이스 정보가 없으면 확인된 해결 방법이 없다고 명확히 안내합니다.
  ...
  public GeneralAgent(ChatClient.Builder builder) {
  ...
  public State execute(State state) {
  	log.info("GeneralAgent 실행");
    String response = chatClient.prompt()
        .user("""
            다음 정보를 참고하여 고객에게 전달할 일반 문의 답변을 작성하세요.
            고객 문의: %s
            문의 분석: %s
            사용자 정보: %s
            지식 베이스 정보: %s
